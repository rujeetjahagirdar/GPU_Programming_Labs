...
void Dense::backward_pass(float learning_rate)
{
    ...
    // grad_in_batch = W * grad_out_batch
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    // set descriptor of output minibatch, out_shape, to have the
    ...
    // create activation descriptor, and set it to have the given
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    cudnnDataType_t dtype;
    int n, c, h, w, n_stride, c_stride, h_stride, w_stride;
    // TODO: Get the input tensor descriptor in_shape into the variables
    //       declared above
    CUDNN_CALL(cudnnGetTensor4dDescriptor(in_shape, &dtype, &n, &c, &h, &w,
        &n_stride, &c_stride, &h_stride, &w_stride));
    ...
    // TODO: Create & set a filter descriptor for a float array ordered
    //       NCHW, w/ shape n_kernels x c x kernel_size x kernel_size.
    //       This is class field filter_desc.
    CUDNN_CALL(cudnnCreateFilterDescriptor(&filter_desc));
    CUDNN_CALL(cudnnSetFilter4dDescriptor(filter_desc, dtype, CUDNN_TENSOR_NCHW,
    ...
    // TODO: Create and set a convolution descriptor. This will
    ...
    CUDNN_CALL(cudnnSetConvolution2dDescriptor(conv_desc, 0, 0, stride, stride,
        1, 1, CUDNN_CONVOLUTION, dtype));
...
Conv2D::~Conv2D()
{
    ...
    // TODO: Destroy filter_desc and conv_desc
...
void Conv2D::forward_pass()
{
    ...
    // TODO: Perform convolution forward pass (store in out_batch).
    ...
    CUDNN_CALL( cudnnConvolutionForward(cudnnHandle,
        &one, in_shape, in_batch,
        filter_desc, weights,
        conv_desc, fwd_algo.algo, workspace, workspace_size,
        &zero, out_shape, out_batch) );
...
void Conv2D::backward_pass(float learning_rate)
{
    ...
    // TODO: Compute the gradient with respect to the filters/weights
    ...
    CUDNN_CALL(cudnnConvolutionBackwardFilter(cudnnHandle,
        &one, in_shape, in_batch,
        out_shape, grad_out_batch, conv_desc, bwd_filter_algo.algo,
    ...
    // TODO: Compute the gradient with respect to the input data
    ...
    CUDNN_CALL(cudnnConvolutionBackwardData(cudnnHandle,
        ...
        out_shape, grad_out_batch, conv_desc, bwd_data_algo.algo,
        workspace, workspace_size,
        &zero, in_shape, grad_in_batch));
    ...
    // TODO: weights = weights + eta * grad_weights
    // CUBLAS_CALL(cublasSaxpy(cublasHandle,
    //     num_filters * filter_height * filter_width * input_channels,
    //     &eta, grad_weights, 1, weights, 1));
    CUBLAS_CALL(cublasSaxpy(cublasHandle,
        n_weights,
        &eta, grad_weights, 1, weights, 1));

    // TODO: biases = biases + eta * grad_biases
    CUBLAS_CALL(cublasSaxpy(cublasHandle,
        n_biases, &eta, grad_biases, 1, biases, 1));
}
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    // TODO: Create and set pooling descriptor to have the given mode,
    ...
    CUDNN_CALL(cudnnSetPooling2dDescriptor(pooling_desc, CUDNN_POOLING_MAX, CUDNN_PROPAGATE_NAN,
        stride, stride, 0, 0, stride, stride));
...
Pool2D::~Pool2D()
{
    // TODO: destroy the pooling descriptor
...
void Pool2D::forward_pass()
{
    ...
    // TODO: do pooling in forward direction, store in out_batch
...
void Pool2D::backward_pass(float learning_rate)
{
    ...
    // TODO: do pooling backwards, store gradient in grad_in_batch
    CUDNN_CALL(cudnnPoolingBackward(cudnnHandle,
        pooling_desc,
        &one, out_shape, out_batch, out_shape,
        grad_out_batch,
        in_shape, in_batch,
        &zero, in_shape, grad_in_batch));
}
